...
bool TOCBlock::FlatDataToBrickedLOD(
  ...
  if (!inFile->Open()) {
    debugOut->Error(_IVDA_func_, "Could not read '%s'!", strSourceFile.c_str());
    return false;
  }
...
bool TOCBlock::FlatDataToBrickedLOD(
  ...
  if (!outFile->Create()) {
    debugOut->Error(_IVDA_func_, "Could not create tempfile '%s'",
                    strTempFile.c_str());
    return false;
  }
  ...
  if(!c.Convert(pSourceData, 0, eType, iComponentCount, vVolumeSize,
    ...
    debugOut->Error(_IVDA_func_, "ExtOctree reported failed conversion.");
    return false;
  }
  outFile->Close(); // note, needed before the 'Open' below!

  pMaxMinDatBlock->SetDataFromFlatVector(statsVec, iComponentCount);
  debugOut->Message(_IVDA_func_, "opening UVF '%s'", m_strDeleteTempFile.c_str());
...
void TOCBlock::GetData(uint8_t* pData, Vec4ui64 coordinates) const {
  m_ExtendedOctree.GetBrickData(pData, coordinates);
}

Vec3ui64 TOCBlock::GetBrickCount(uint64_t iLoD) const {
  return m_ExtendedOctree.GetBrickCount(iLoD);
}

Vec3ui64 TOCBlock::GetBrickSize(Vec4ui64 coordinates) const {
  return m_ExtendedOctree.ComputeBrickSize(coordinates);
}

const TOCEntry& TOCBlock::GetBrickInfo(Vec4ui64 coordinates) const {
  return m_ExtendedOctree.GetBrickToCData(coordinates);
}

Vec3d TOCBlock::GetBrickAspect(Vec4ui64 coordinates) const {
  return m_ExtendedOctree.GetBrickAspect(coordinates);
}

Vec3ui64 TOCBlock::GetLODDomainSize(uint64_t iLoD) const {
...
Vec3d TOCBlock::GetScale() const {
  return m_ExtendedOctree.GetGlobalAspect();
}

void TOCBlock::SetScale(const Vec3d& scale) {
  m_ExtendedOctree.SetGlobalAspect(scale);
}

uint64_t TOCBlock::GetLinearBrickIndex(Vec4ui64 coordinates) const {
