  ...
  m_Root(0)
{
  ...
  Subdivide( m_Root, Vec3d(m_mesh->m_Bounds[0]),
                     Vec3d(m_mesh->m_Bounds[1]), m_maxDepth );
  std::cout << "Done" << std::endl;
...
struct StackElem {
  ...
  Vec3d pb;          // the coordinates of entry/exit point 
  int prev;          // the pointer to the previous stack item 
};

double KDTree::Intersect(const Ray& ray, Vec3f& normal,
  ...
  // traverse kd-tree
    ...
    Vec3f _normal;   Vec2f _tc; Vec4f _color;
...
void KDTree::Subdivide(KDTreeNode* node, const Vec3d& min,
                       const Vec3d& max, int recDepth) {
	// determine split axis (always split along the longest axis)
	Vec3d bboxSize = max-min;
  ...
  for (slist::iterator candidate = splitCandidates.begin();
    ...
    Vec3d b1 = bboxSize;  b1[axis] = candidate->pos - min[axis];
    Vec3d b2 = bboxSize;  b2[axis] -= b1[axis];
  ...
  node->SetSplitPos(bestpos);
	...
	if (recDepth > 1)
	{
    Vec3d max1 = max; max1[axis] = bestpos;
		if (left->GetList().size() > 2) Subdivide( left,  min,  max1, recDepth-1);
    Vec3d min2 = min; min2[axis] = bestpos;
