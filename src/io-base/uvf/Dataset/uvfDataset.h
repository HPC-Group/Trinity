...
#include "UVF-File/RasterDataBlock.h"
#include "UVF-File/MaxMinDataBlock.h"
#include "UVF-File/TOCBlock.h"
...
public:
  ...
  Core::Math::Vec3ui                  m_aOverlap;
  /// size of the domain for this timestep (i.e. n_voxels in finest LOD)
  std::vector<Core::Math::Vec3ui64>   m_aDomainSize;
  /// max values for logical brick indices; std::vector index gives LOD.
  std::vector<Core::Math::Vec3ui64>   m_vaBrickCount;
  /// the size of each individual brick.  Slowest moving dimension is LOD;
  /// then x,y,z.
  std::vector<std::vector<std::vector<std::vector<Core::Math::Vec3ui64>>>>  m_vvaBrickSize;
...
public:
  ...
  virtual Core::Math::Vec3ui64 GetEffectiveBrickSize(const BrickKey &) const;
  ...
  virtual Core::Math::Vec3ui64 GetDomainSize(const size_t lod=0, const size_t ts=0) const;
  ///@}
  virtual uint64_t GetNumberOfTimesteps() const;
  
  Core::Math::Vec3ui GetBrickLayout(size_t lod, size_t ts) const;
  ...
  virtual bool Crop(const Core::Math::PLANE<float>& plane,
                    const std::string& strTempDir,
                    bool bKeepOldData, bool bUseMedianFilter,
                    bool bClampToEdge, uint64_t incoreSize);
  ...
  bool GeometryTransformToFile(size_t iMeshIndex, const Core::Math::Mat4f& m, const Core::Math::Vec4f& c);
  ...
  Core::Math::Vec4ui64 KeyToTOCVector(const BrickKey &k) const;
  BrickKey TOCVectorToKey(const Core::Math::Vec4ui& hash, size_t timestep) const;
  ...
  virtual std::pair<Core::Math::Vec3f, Core::Math::Vec3f>
...
private:
  ...
  Core::Math::Vec3ui                           m_aMaxBrickSize;
  ...
  Core::Math::Vec3f GetVolCoord(uint64_t pos, const Core::Math::Vec3ui64& domSize) {
    Core::Math::Vec3ui64 domCoords;
    
    domCoords.x = pos % domSize.x;
    domCoords.y = (pos / domSize.x) % domSize.y;
    domCoords.z = pos / (domSize.x*domSize.y);
    
    Core::Math::Vec3f normCoords(float(domCoords.x) / float(domSize.x),
                                 float(domCoords.y) / float(domSize.y),
                                 float(domCoords.z) / float(domSize.z));
    normCoords -= Core::Math::Vec3f(0.5, 0.5, 0.5);
    return normCoords;
  }
  
  template<typename T>
  bool CropData(LargeRAWFile& dataFile, const Core::Math::PLANE<float>& plane,
                const Core::Math::Vec3ui64& domSize,
                const uint64_t iComponentCount, uint64_t incoreSize) {
    
    assert(iComponentCount == size_t(iComponentCount));
    
    size_t iInCoreElemCount = size_t(incoreSize/sizeof(T));
    ...
    do {
      ...
      for (size_t elem = 0;elem<iElemsRead;elem+=size_t(iComponentCount)) {
        Core::Math::Vec3f volCoord = GetVolCoord((iFilePos/sizeof(T)+elem)/iComponentCount, domSize);
      ...
      LINFO("Cropping voxels ("<<(100.0f*float(iFilePos)/float(iFileSize))<<"% completed)");
