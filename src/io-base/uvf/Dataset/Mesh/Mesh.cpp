...
Mesh::Mesh(const BasicMeshData& bmd,
  ...
  const Core::Math::Vec4f& defColor) :
  m_KDTree(0),
  m_Data(bmd),
  m_DefColor(defColor),
  m_MeshDesc(desc),
...
void Mesh::ComputeUnitCubeScale(Core::Math::Vec3f& scale, 
                                Core::Math::Vec3f& translation) {
  if (m_Data.m_vertices.empty()) {
    translation = Core::Math::Vec3f(0,0,0);
    scale = Core::Math::Vec3f(1,1,1);
    return;
  }

  Core::Math::Vec3f maxExtensionV = (m_Bounds[1]-m_Bounds[0]);
...
void Mesh::ScaleAndBias(const Core::Math::Vec3f& scale,
  ...
  Core::Math::Mat4f s;  s.Scaling(scale);
  Core::Math::Mat4f b;  b.Translation(translation);
...
void Mesh::ScaleToUnitCube() {
  Core::Math::Vec3f scale, translation;
...
void Mesh::RecomputeNormals() {
  ...
  for(size_t i = 0;i<m_Data.m_normals.size();i++) m_Data.m_normals[i] = Core::Math::Vec3f();

  for (size_t i = 0;i<m_Data.m_VertIndices.size();i+=3) {
    Core::Math::Vec3ui indices(m_Data.m_VertIndices[i], m_Data.m_VertIndices[i+1], m_Data.m_VertIndices[i+2]);

    Core::Math::Vec3f tang = m_Data.m_vertices[indices.x]-m_Data.m_vertices[indices.y];
    Core::Math::Vec3f bin  = m_Data.m_vertices[indices.x]-m_Data.m_vertices[indices.z];

    Core::Math::Vec3f norm = bin % tang;
...
double Mesh::IntersectInternal(const Ray& ray, Core::Math::Vec3f& normal,
  ...
  if (m_KDTree) {
    ...
    Core::Math::Vec3f _normal;
    Core::Math::Vec2f _tc;
    Core::Math::Vec4f _color;
...
double Mesh::IntersectTriangle(size_t i, const Ray& ray, 
  ...
  Core::Math::Vec3f vert0 = m_Data.m_vertices[m_Data.m_VertIndices[i]];
  Core::Math::Vec3f vert1 = m_Data.m_vertices[m_Data.m_VertIndices[i+1]];
  Core::Math::Vec3f vert2 = m_Data.m_vertices[m_Data.m_VertIndices[i+2]];

  // find vectors for two edges sharing vert0
  Core::Math::Vec3d edge1 = Core::Math::Vec3d(vert1 - vert0);
  Core::Math::Vec3d edge2 = Core::Math::Vec3d(vert2 - vert0);
   
  // begin calculating determinant - also used to calculate U parameter
  Core::Math::Vec3d pvec = ray.direction % edge2;
  ...
  Core::Math::Vec3d tvec = ray.start - Core::Math::Vec3d(vert0);
  ...
  Core::Math::Vec3d qvec = tvec % edge1;
  ...
  if (m_Data.m_NormalIndices.size()) {
    Core::Math::Vec3f normal0 = m_Data.m_normals[m_Data.m_NormalIndices[i]];
    Core::Math::Vec3f normal1 = m_Data.m_normals[m_Data.m_NormalIndices[i+1]];
    Core::Math::Vec3f normal2 = m_Data.m_normals[m_Data.m_NormalIndices[i+1]];

    Core::Math::Vec3f du = normal1 - normal0;
    Core::Math::Vec3f dv = normal2 - normal0;
    
    normal = normal0 + du * float(u) + dv * float(v);
  } else {
    // compute face normal if no normals are given
    normal = Core::Math::Vec3f(edge1 % edge2);
  }
  normal.normalize();

  if ((Core::Math::Vec3f(ray.direction) ^ normal) > 0) normal = normal *-1; 

  // interpolate texture coordinates
  if (m_Data.m_TCIndices.size()) {
    Core::Math::Vec2f tc0 = m_Data.m_texcoords[m_Data.m_TCIndices[i]];
    Core::Math::Vec2f tc1 = m_Data.m_texcoords[m_Data.m_TCIndices[i+1]];
    Core::Math::Vec2f tc2 = m_Data.m_texcoords[m_Data.m_TCIndices[i+2]];
  ...
  if (m_Data.m_COLIndices.size()) {
    Core::Math::Vec4f col0 = m_Data.m_colors[m_Data.m_TCIndices[i]];
    Core::Math::Vec4f col1 = m_Data.m_colors[m_Data.m_TCIndices[i+1]];
    Core::Math::Vec4f col2 = m_Data.m_colors[m_Data.m_TCIndices[i+2]];
...
bool Mesh::AABBIntersect(const Ray& r, double& tmin, double& tmax) const {
  ...
  Core::Math::Vec3d inv_direction(1.0/r.direction.x, 
                          1.0/r.direction.y, 
                          1.0/r.direction.z);
