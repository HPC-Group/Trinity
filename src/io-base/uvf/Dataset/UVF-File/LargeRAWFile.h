...
#include <string>
#include <vector>
#include "silverbullet/math/EndianConvert.h"
...
public:
  ...
  template<class T> void ReadData(T& value, bool bIsBigEndian) {
    ...
    if (Core::Math::EndianConvert::IsBigEndian() != bIsBigEndian)
      Core::Math::EndianConvert::Swap<T>(value);
  }
  ...
  template<class T> void ReadData(std::vector<T> &value, uint64_t count,
    ...
    if (Core::Math::EndianConvert::IsBigEndian() != bIsBigEndian) {
      for (size_t i = 0; i < count; i++) {
        Core::Math::EndianConvert::Swap<T>(value[i]);
      }
    }
  }
