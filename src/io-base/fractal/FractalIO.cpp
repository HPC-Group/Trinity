...
namespace trinity {
  ...
  m_mbGenerator(0,0,0), m_listData(listData)  // HACK: worthless init which will be overriden below
  {
    switch (fileId) {
      case 3:
        pseudoConstructor(64, 64, 64, 64, 64, 64); break;
      case 4:
        pseudoConstructor(128, 128, 128, 128, 128, 128); break;
      case 5:
        pseudoConstructor(256, 256, 256, 256, 256, 256); break;
      case 6:
        pseudoConstructor(512, 512, 512, 32, 32, 32); break;
      case 7:
        pseudoConstructor(1024, 1024, 1024, 32, 32, 32); break;
      case 8:
        pseudoConstructor(2048, 2048, 2048, 32, 32, 32); break;
      case 9:
        pseudoConstructor(4096, 4096, 4096, 32, 32, 32); break;
      default:
  ...
  void FractalIO::pseudoConstructor(uint64_t sx, uint64_t sy, uint64_t sz,
  ...
  bool FractalIO::getBrick(const BrickKey& key,
      ...
      for (int z = 0;z<m_totalSize.z;++z) {
        for (int y = 0;y<m_totalSize.y;++y) {
          for (int x = 0;x<m_totalSize.x;++x) {
            const size_t i = size_t(x+y*m_totalSize.x+
                                    z*m_totalSize.x*m_totalSize.y);
