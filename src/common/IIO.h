...
namespace trinity {
  ...
  public:
    ...
    virtual std::shared_ptr<std::vector<uint8_t>> getBrick(const BrickKey& brickKey, bool& success) const = 0;
    virtual std::vector<std::shared_ptr<std::vector<uint8_t>>> getBricks(const std::vector<BrickKey>& brickKeys, bool& success) const = 0;
