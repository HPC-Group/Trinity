...
void ProcessingNode::run() {
    ...
    while (!isInterrupted()) {
        ...
        if (!msgEnvelope.isNull()) {          // req arrived
            ...
            m_aggregator.send(
                MessageEnvelope(std::move(mocca::ByteArray() << reply.str()), env.senderID));
        }
    }
}
...
std::string ProcessingNode::handleSpawnRendererCmd(std::vector<std::string>& args) {
    ...
    std::string ret = vcl::TRI_RET + "_0_" + args[2] + "_" + std::to_string(session->getSid()) +
                      "_" + std::to_string(session->getPort());

    // endpoint for the render session
    std::unique_ptr<mocca::net::Endpoint> ep(
        new mocca::net::Endpoint(m_endpoint.protocol(), std::to_string(session->getPort())));
