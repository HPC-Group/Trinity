...
GLGridLeaper2::GLGridLeaper2(std::shared_ptr<Context::Context> context,Vec2ui vWinSize, ERenderMode mode) :
...
bool GLGridLeaper2::Initialize(uint64_t gpuMemorySizeInByte){
	...
	m_pRenderThread = std::make_shared<std::thread>(&GLGridLeaper2::run, this);
...
//! cleanup
void GLGridLeaper2::Cleanup(){
    CleanupShader();
    CleanupTraversalShader();
    CleanupFrameBuffer();
    CleanupGeometry();
}
void GLGridLeaper2::CleanupShader(){
...
void GLGridLeaper2::CleanupTraversalShader(){
...
void GLGridLeaper2::CleanupFrameBuffer(){
...
void GLGridLeaper2::CleanupGeometry(){
	m_pVolumeBox.reset();
	m_pNearPlane.reset();
}
...
void GLGridLeaper2::SwitchPagingStrategy(MissingBrickStrategy brickStrategy){
	CleanupTraversalShader();
...
void GLGridLeaper2::SetViewPort(Core::Math::Vec2ui lower_left, Core::Math::Vec2ui upper_right,
...
//! ----- Shader Init ------------------------------------------
bool GLGridLeaper2::LoadTraversalShader(MissingBrickStrategy brickStrategy){
...
bool GLGridLeaper2::LoadFrontFaceShader(){
	std::vector<std::string> vs, fs;
...
bool GLGridLeaper2::LoadComposeShader(){
	std::vector<std::string> vs, fs;
...
bool GLGridLeaper2::LoadCheckShader(GLCore::GLProgramPtr& programptr, ShaderDescriptor& sd, std::string name){
...
bool GLGridLeaper2::InitOffScreenBuffers(){
...
void GLGridLeaper2::swapBuffers(bool clearview){
    if(!clearview){
        m_pFBORayStart = m_pFBORayStartMAIN;
        m_pFBORayStartNext = m_pFBORayStartNextMAIN;
        m_pFBOStartColor = m_pFBOStartColorMAIN;
...
void GLGridLeaper2::recreateFBO(){
	CleanupFrameBuffer();
	InitOffScreenBuffers();
}

bool GLGridLeaper2::RecreateAfterResize(){
...
void GLGridLeaper2::InitHashTable() {
...
void GLGridLeaper2::InitVolumePool(uint64_t gpuMemorySizeInByte){
...
void GLGridLeaper2::InitGeometry(){
...
//! Render Entrance
void GLGridLeaper2::FillRayEntryBuffer(){
...
//! Raycast
void GLGridLeaper2::Raycast(Tuvok::Renderer::ERenderMode rendermode){
...
//swap to next buffer
void GLGridLeaper2::swapToNextBuffer(bool clearview){
if(!clearview){
...
void GLGridLeaper2::SelectShader(Tuvok::Renderer::ERenderMode rendermode){
...
void GLGridLeaper2::SetupRaycastShader(Tuvok::Renderer::ERenderMode rendermode){
...
//! Final Compose
void GLGridLeaper2::FinalCompose(){
	GLenum err = glGetError();
...
//! Helper Functions ------------------------------------------------
Vec4ui GLGridLeaper2::RecomputeBrickVisibility(bool bForceSynchronousUpdate) {
...
uint32_t GLGridLeaper2::UpdateToVolumePool(const Vec4ui& brick) {
	std::vector<UINTVECTOR4> bricks;
	bricks.push_back(brick);
	return UpdateToVolumePool(bricks);
}

uint32_t GLGridLeaper2::UpdateToVolumePool(std::vector<Vec4ui>& hash) {
...
void GLGridLeaper2::computeMathMember(){
...
//! Set Functions --------------------------------------------------
void GLGridLeaper2::SetRenderMode(ERenderMode mode){
	AbstrRenderer::SetRenderMode(mode);
	//! \todo find out what to do
}

void GLGridLeaper2::Set1DTransferFunctionptr(GLCore::GLTexture1DPtr texturePtr){
	m_pTransferFunctionTexture = texturePtr;
}

void GLGridLeaper2::ClipVolume(Core::Math::Vec3f minValues, Core::Math::Vec3f maxValues){
	m_pVolumeBox->SetSlideLevels(minValues, maxValues);
	m_bCompleteRedraw = true;
}

void GLGridLeaper2::Set1DTransferFunction(std::vector<Core::Math::Vec4f> data){
...
void GLGridLeaper2::CalculateUsedGPUMemory(){
	m_UsedGPUMemory = 0;
...
Core::Math::Vec3ui GLGridLeaper2::CalculateVolumePoolSize(uint64_t GPUMemorySizeInByte){
...
void GLGridLeaper2::ReadFrameBuffer(std::vector<uint8_t>& pixels, int& width, int& height, int& componentCount){
...
void GLGridLeaper2::makeScreenshot(){
    screenshot(0);
}

Core::Math::Vec4f GLGridLeaper2::readVolumePosition(Core::Math::Vec2ui v){
