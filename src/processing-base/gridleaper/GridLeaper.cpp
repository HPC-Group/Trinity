...
void GridLeaper::LoadFrameBuffers() {
    m_resultBuffer = std::make_shared<GLFBOTex>(GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE, m_width, m_height, GL_RGBA, GL_RGBA,
                                                GL_UNSIGNED_BYTE, true, 1);

    // TODO: check if we need depth (the true below)
    m_backfaceBuffer =
        std::make_shared<GLFBOTex>(GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE, m_width, m_height, GL_RGBA, GL_RGBA, GL_FLOAT, true, 1);
}
...
void GridLeaper::paintInternal(PaintLevel paintlevel) {
    ...
    projection.Perspective(45.0f, (float)m_width / (float)m_height, 0.01f, 1000.0f);
    ...
    m_resultBuffer->ReadBackPixels(0, 0, m_width, m_height, m_bufferData.data());
