...
bool GridLeaper::LoadShader()
{
    ...
    if(!LoadAndCheckShaders(_sampleShader,sd)) return false;
}
...
{
    _sampleFrameBuffer = std::make_shared<GLFBOTex>(GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE, _width, _height, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, true, 1);
}
