...
void GridLeaper::paintInternal(PaintLevel paintlevel) {
  ...
  // if metadata just became active, promote paintlevel
  if(m_volumePool->isMetaDataReady() && !m_volumePool->IsVisibilityUpdated())
    paintlevel = IRenderer::PaintLevel::PL_REDRAW_VISIBILITY_CHANGE;
  
  // fall through on purpose
  switch (paintlevel) {
    case IRenderer::PaintLevel::PL_REDRAW_VISIBILITY_CHANGE :
      if(m_volumePool->isMetaDataReady()) RecomputeBrickVisibility(false);
