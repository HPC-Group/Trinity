...
ProcessingNodeProxy::initRenderer(const VclType& type,
                                int fileId,
                                const mocca::net::Endpoint& endpoint,
                                const StreamingParams& params) {
    
    // creating the cmd that will initialize a remote renderer of the given type
    InitProcessingSessionCmd cmd(0,
                        m_ridGen.nextID(),
                        m_inputChannel.getEndpoint().protocol,
                        type,
                        fileId,
                        endpoint.toString(),
                        params);
...
std::unique_ptr<RendererProxy>
ProcessingNodeProxy::handleInitRendererReply(const ReplyInitProcessingSessionCmd& replyCmd,
                                             const StreamingParams& params) {
    ...
    std::unique_ptr<RendererProxy> ren(new RendererProxy(stream,
