...
void Window::initRenderer() {
    ...
    try {
        m_renderer = _processingNode->initRenderer(trinity::VclType::SimpleRenderer, m_dataID, endpointIO, params);
    } catch (const trinity::TrinityError&) {
...
void Window::on_IOconnectIP_clicked() {
    ...
    ui->dsWidget->setModel(new IODataModel(*_ioNode, this));
    connect(ui->dsWidget->selectionModel(), SIGNAL(currentChanged(const QModelIndex&, const QModelIndex&)), this,
...
void Window::currentItemChanged(const QModelIndex& current, const QModelIndex& previous) {
    if (auto ioData = current.data(Qt::UserRole + 1).value<trinity::IOData*>()) {
        m_dataID = ioData->getFileId();
    }
}