...
public:
  ...
  void GetGeometry(VertVec& vertices, NormVec& normals,
    ...
    Vec3ui iVertices1(sVertices,sVertices+1,sVertices+3);
    Vec3ui iVertices2(sVertices+2,sVertices+3,sVertices+0);
    ...
    if (!m_bIsLeaf && iDepth > 0) {
      Vec3f max1 = max; max1[m_Axis] = float(m_SplitPos);
      Vec3f min2 = min; min2[m_Axis] = float(m_SplitPos);
...
public:
  ...
  void RescaleAndShift(const Vec3f& translation,
                       const Vec3f& scale) {
...
private:
  ...
  void Subdivide(KDTreeNode* node, const Vec3d& min,
