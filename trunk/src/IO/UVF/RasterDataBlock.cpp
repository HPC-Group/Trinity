                                ...
                                uint64_t iMaxLODLevel)
{
    ...
    if (i==0 || iWindowTargetPos >= iTargetWindowSize) {
      if (i==0) {
        ...
        if(pTargetFile->WriteRAW(pTargetData, bytes) != bytes) {
          pDebugOut->Error(_IVDA_func_, "short write while subsampling!");
          assert(1 == 0);
        }
      }
      ...
      if(pDebugOut && (--uCount == 0)) {
        ...
        pDebugOut->Message(_IVDA_func_, "Generating data for lod level %i of %i:"
      ...
      if (pSourceFile == pTargetFile) {
        ...
        if(pSourceFile->ReadRAW(pSourceData, bytes) != bytes) {
          pDebugOut->Error(_IVDA_func_, "short read from '%s'",
        ...
        pSourceFile->SeekPos(iSourcePos*iElementSize);
        if(pSourceFile->ReadRAW(pSourceData, bytes) != bytes) {
          pDebugOut->Error(_IVDA_func_, "short read from data");
        }
      }
  ...
  const uint64_t bytes = iTargetWindowSize*iElementSize;
  if(pTargetFile->WriteRAW(pTargetData, bytes) != bytes) {
    pDebugOut->Error(_IVDA_func_, "short write to '%s'",
  ...
  AbstrDebugOut* pDebugOut)
{
  ...
  LargeRAWFile pSourceFile(AppendFilename(strTempFile,"0"));
  ...
  AbstrDebugOut* pDebugOut)
{
  ...
  for (size_t i = 0;i<ulBrickSize.size();i++)  {
    if (ulBrickSize[i] < ulBrickOverlap[i]) {
      if (pDebugOut) pDebugOut->Error(_IVDA_func_, "Invalid parameters: Bricksze is smaler than brick overlap");
      return false;
    }
  }
  ...
  if (!m_pTempFile) {
    AllocateTemp(AppendFilename(strTempFile,"1"),
                 m_vLODOffsets.empty());
  }
  ...
  for (size_t i = 0;i<vLODCombis.size();i++) {
    ...
    if (i > 0) {
      if (i > 1) {
        ...
        tempFile = LargeRAWFile_ptr(new LargeRAWFile(AppendFilename(strTempFile,"2")));
        if (!tempFile->Create(ComputeDataSize())) {
          if (pDebugOut) pDebugOut->Error(_IVDA_func_, "Unable To create Temp File");
          return false;
        }
    ...
    for (size_t j=0;j<vBrickPermutation.size();j++) {
       ...
       if (pDebugOut) pDebugOut->Message(_IVDA_func_, "Processing brick %i of %i in lod level %i of %i",int(j+1),int(vBrickPermutation.size()),int(i+1), int(vLODCombis.size()));
      ...
      vector<Vec4d> fMinMax(static_cast<size_t>(ulElementDimension));
      for (uint64_t k=0;k<iBrickSize/vBrickPermutation[j][0];k++) {
        ...
        for (uint64_t l = 0;l<iDataSize;l+=BLOCK_COPY_SIZE) {
          ...
          if(bytes != iCopySize) {
            IVDA_ERRORV("Error reading data from %s!",
...
bool RasterDataBlock::BrickedLODToFlatData(const vector<uint64_t>& vLOD, const std::string& strTargetFile,
  ...
  if (!pTargetFile->IsOpen()) {
    if (pDebugOut) pDebugOut->Error(_IVDA_func_,"Unable to write to target file %s.", strTargetFile.c_str());
    return false;
  }
...
RasterDataBlock::TraverseBricksToApplyFunction(
  ...
  if (iCurrentDim>0) {
    ...
    for (size_t i = 0;i<vBrickCount[0];i++) {
      ...
      if (pDebugOut) pDebugOut->Message(_IVDA_func_, "Extracting volume data\nProcessing brick %i of %i",int(++iBrickCounter),int(iBrickCount));
      ...
      if (pDebugOut) pDebugOut->Message(_IVDA_func_, "Processing volume data\nProcessing brick %i of %i",int(iBrickCounter),int(iBrickCount));

      if (!brickFunc(&vDataOverlapFixed[0], Vec3ui64(vEffectiveBrickSize), Vec3ui64(vAbsCoords), pUserContext )) return false;
...
RasterDataBlock::TraverseBricksToWriteBrickToFile(
  ...
  if (iCurrentDim>0) {
    ...
    for (size_t i = 0;i<vBrickCount[0];i++) {
      ...
      if (pDebugOut) pDebugOut->Message(_IVDA_func_, "Processing brick %i of %i",int(++iBrickCounter),int(iBrickCount));
